...
//PID constants
int kp = 9.1;   int ki = 0.99;   int kd = 1.8;
int PID_pp = 0;    int PID_ip = 0;    int PID_dp = 0;
int PID_pr = 0;    int PID_ir = 0;    int PID_dr = 0;
int flag=0;
...
void setup() {
  ...
  Serial.println(F("  MPU-6050 IMU"));                                         //Print text to screen
  //lcd.setCursor(0,1);                                                  //Set the LCD cursor to position to position 0,1
  Serial.println(F("     V1.0"));                                              //Print text to screen

  delay(1500);                                                         //Delay 1.5 second to display the text
  //lcd.clear();                                                         //Clear the LCD

  //lcd.setCursor(0,0);                                                  //Set the LCD cursor to position to position 0,0
  Serial.println(F("Calibrating gyro"));                                       //Print text to screen
  //lcd.setCursor(0,1);                                                  //Set the LCD cursor to position to position 0,1
...
void loop() {
  ...
  if(count>50 && flag==0)
  {
    setX = angle_pitch_output;
    setY = angle_roll_output;
    flag=1;
  }
...
void write_LCD() {
  pitchError = setX - angle_pitch_output;
  rollError = setY - angle_roll_output;
  Serial.print(angle_pitch_output);
  Serial.print(F("  "));
  Serial.print(angle_roll_output);
  Serial.print(F("  "));
  PID_pp = kp * pitchError;
  PID_pr = kp * rollError;

  //Calculate the I value in a range on +-3
  if (-0.5 < pitchError < 0.5)
  {
    PID_ip = PID_ip + (ki * pitchError);
  }
  if (-0.5 < rollError < 0.5)
  {
    PID_ir = PID_ir + (ki * rollError);
  }
  ...
  PID_dp = kd * ((pitchError - previous_errorp) / elapsedTime);
  PID_dr = kd * ((rollError - previous_errorr) / elapsedTime);

  //Final total PID value is the sum of P + I + D
  PID_valuep = PID_pp + PID_ip + PID_dp;
  PID_valuer = PID_pr + PID_ir + PID_dr;

  Serial.print(F(" Pitch : "));
  Serial.print(PID_valuep);
  Serial.print(F("\tRoll : "));
  Serial.print(PID_valuer);

  if (PID_valuep > 20)
  {
    for (int i = 0; i < PID_valuep; i++)
    {
  ...
  if (PID_valuep < -20)
  {
    for (int i = 0; i < -PID_valuep; i++)
    {
      step(true, Z_DIR, Z_STP, 1); //X, Clockwise
